...
- **Zoe's reading** — \`lib/games/sightwords.ts\` (Edmark Level 1 words in lesson order; \`WORD_EMOJI\` supplies picture cues). Three games (flashcards, picture→word, hear→type) with browser TTS audio, +5/day (Zoe only). The active word range and per-word mastery (correct/incorrect attempts) live in Redis and are managed from **/admin** — add the next word with a click. \`PRACTICE_THROUGH_LESSON\` is just the default before any admin change.
